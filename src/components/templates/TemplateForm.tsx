...
/**
 * Compute initial form data from template prop
 */
function getInitialFormData(template?: Template): TemplateFormData {
	if (template) {
		return {
			name: template.name,
...
export function TemplateForm({
	...
	// Use template.id as key to reset form when switching between templates
	...
	const handlePlatformToggle = (platformId: PlatformId) => {
		setFormData((prev) => {
			...
			if (isSelected) {
				...
				return {
					...prev,
					platformSupport: prev.platformSupport.filter(
						(id) => id !== platformId
					),
					platformPrompts: prev.platformPrompts.filter(
	...
	const getPromptForPlatform = (platformId: PlatformId): string => {
		const platformPrompt = formData.platformPrompts.find(
			(p) => p.platformId === platformId
		);
	...
	const validateForm = (): boolean => {
		...
		if (missingPrompts) {
			newErrors.platformPrompts =
				'All selected platforms must have a prompt';
		}
					...
					<div className='space-y-3'>
						<div>
							...
							<p className='text-sm text-muted-foreground mt-0.5'>
								Select the platforms this template will be used
								for
							</p>
						</div>
						<div className='flex flex-wrap gap-3'>
							{platforms.map((platform) => {
								...
								const isSelected =
									formData.platformSupport.includes(
										platform.id
									);
								return (
									<Button
					...
					{formData.platformSupport.length > 0 && (
						<div className='space-y-4'>
							<div>
								...
								<p className='text-sm text-muted-foreground mt-0.5'>
									Define a custom prompt for each selected
									platform
								</p>
							</div>

							<div className='space-y-4'>
								{formData.platformSupport.map(
									(platformId, index) => {
										const platform =
											getPlatformConfig(platformId);
										const Icon = platform.icon;
										const prompt =
											getPromptForPlatform(platformId);
